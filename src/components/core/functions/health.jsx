...
export const updateHealthDetails = (data = {}) => {
  try {
    ...
    data.dailyCalorie = isNaN(defaultValues?.formaintaining * calories) ? 0 : parseFloat(((defaultValues?.formaintaining * calories) / 100).toFixed(2));
    data.percentageOfProtein = isNaN(percentageOfProtein) ? 0 : parseFloat(percentageOfProtein.toFixed(2));
  } catch (error) {
    console.log(error);
  }
};
...
export const updateCaloriDetails = (data = {}) => {
  try {
    ...
    data.dailyCalorie = isNaN(defaultValues?.formaintaining * calories) ? 0 : (defaultValues?.formaintaining * calories) / 100 ? 0 : percentageOfProtein;
...
export const updateDailyCaloric = (value = {}) => {
  ...
  try {
    ...
    const isValue = Object.values(defaultValues).every((defaultValue) => value?.proposedCalorie !== defaultValue);
