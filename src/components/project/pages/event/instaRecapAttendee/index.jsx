...
const InstaRecapAttendee = (props) => {
  const { id } = props;
    ...
    (eventForm) => {
      ...
      const formFields = eventForm.map((attribute) => {
        ...
        if (["user"].includes(formattedAttribute.name)) {
          ...
          formattedAttribute.collection = "recapUser";
        }
  ...
  useEffect(() => {
    document.title = `${title} - Recap User`;
    getData({ event: props?.openData?.data?._id }, "recap-user-form").then((response) => {
  ...
  const processData = useCallback((data) => {
    ...
    data.forEach((match) => {
      ...
      if (!userMap.has(userId) || new Date(match.date) > new Date(userMap.get(userId).date)) {
  ...
  return (
    ...
    selectedTab === "all" && (
      <ListTable
        api="ticket-registration/recap-user"
        // key={`recap-user-${id}`}
        itemTitle={{
          name: "value",
          type: "text",
...
export default Layout(InstaRecapAttendee);
